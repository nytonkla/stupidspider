...
// According to http://stackoverflow.com/questions/2247977/what-does-zend-mm-heap-corrupted-mean
ini_set('output_buffering', 8192);

// Turn all error into PHP exception.
// Taken from http://stackoverflow.com/questions/1241728/can-i-try-catch-a-warning
...
class Parser2 extends CI_Controller {
    
	function __construct()
	{
		parent::__construct();
		log_message('info'," PARSER : Turn all errors into PHP exception");
	...
	function __destruct()
	{
		log_message('info', "Restore error handler to basic CI's one");
...
//				$meta = $e->find('font[color]',count($e->find('font[color]'))-1);
	...
	function parse_pantip_cafe($fetch,$page)
	{
		...
		foreach($elements as $e)
		{
			...
			$special_date = array('วันสิ้นปี' => '31 ธ.ค.','วันปีใหม่' => '1 ม.ค.', 'วันนักประดิษฐ์' => '2 ก.พ.', 'วันวาเลนไทน์' => '14 ก.พ.','วันเข้าพรรษา'=>'27 ก.ค.','วันแม่แห่งชาติ'=>'12 ส.ค.','วันเกิด PANTIP.COM'=>'7 ต.ค.','วันปิยมหาราช'=>'23 ต.ค.','วันลอยกระทง'=>'10 พ.ย.','วันพ่อแห่งชาติ'=>'5 ธ.ค.','วันจักรี'=>'6 เม.ย.','วันฉัตรมงคล'=>'5 พ.ค.','วันมาฆบูชา'=>'7 มี.ค.','วันสตรีสากล'=>'8 มี.ค.');
	...
	function parse($fetch,$domain_id)
	{
		if ($domain_id == 15 || $domain_id == 29 || $domain_id == 16 || $domain_id == 21 || $domain_id == 18 || $domain_id == 10 || $domain_id == 13 || $domain_id == 4 || $domain_id == 25 || $domain_id == 26 || $domain_id == 27 || $domain_id == 20 || $domain_id == 28 || $domain_id >= 30 || $domain_id == 1 || $domain_id == 3 || $domain_id == 12)
		{
			// load helper file from table domain.name
			$domain = new Domain_model();
			$domain->init($domain_id);
...
//				else { echo "file not null"; }
				...
				if ($page->domain_id == 15 || $page->domain_id == 29 || $page->domain_id == 16 || $page->domain_id == 21 || $page->domain_id == 18 || $page->domain_id == 10 || $page->domain_id == 13 || $page->domain_id == 4 || $page->domain_id == 25 || $page->domain_id == 26 || $page->domain_id == 27 || $page->domain_id == 20 || $page->domain_id == 28 || $page->domain_id >= 30 || $page->domain_id == 1 || $page->domain_id == 3 || $page->domain_id == 12)
				{
					// load helper file from table domain.name
					$domain = new Domain_model();
	...
	function test($id,$type=null)
	{
		...
		else
		{
			...
			if($page->domain_id == 15 || $page->domain_id == 29 || $page->domain_id == 16 || $page->domain_id == 21 || $page->domain_id == 18 || $page->domain_id == 10 || $page->domain_id == 13 || $page->domain_id == 25 || $page->domain_id == 26 || $page->domain_id == 27 || $page->domain_id == 4 || $page->domain_id == 20 || $page->domain_id == 28 || $page->domain_id >= 30 || $page->domain_id == 1 || $page->domain_id == 3 || $page->domain_id == 12)
			{
