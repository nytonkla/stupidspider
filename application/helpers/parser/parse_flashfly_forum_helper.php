<?php
	function parse_flashfly_forum($fetch,$page,$debug=false)
	{
		...
		else
		{
			if($parsed_posts_count == 0 && $page->sub_comment == 0) // No early post and not a sub comment page
			{
				...
				$post_date = trim(str_replace(array(",","am","pm"),"",$post_date));
							
				$date = explode(" ",$post_date);
				$l = count($date);
				
				$post_date = $date[$l-2]."-".enMonth_decoder($date[$l-4],"cut")."-".$date[$l-3]." ".$date[$l-1];
			...
			foreach($comments as $k=>$c)
			{
				...
				$comment_date = trim(str_replace(array(",","am","pm"),"",$comment_date));
							
				$date = explode(" ",$comment_date);
				$l = count($date);
				
				$comment_date = $date[$l-2]."-".enMonth_decoder($date[$l-4],"cut")."-".$date[$l-3]." ".$date[$l-1];
