<?php
	function parse_mom2kids_forum($fetch,$page,$debug=false)
	{
		$log_unit_name = 'parse_mom2kids_forum';
		...
		else
		{
			...
			{
				$main_content = $html->find('.windowbg td[valign=middle] a');
				...
				$author = $html->find('.windowbg td[valign=top] b');
				...
				$p_view = $html->find('.catbg3 td[id=top_subject]',0);
				...
				$mm = enMonth_decoder($pdate[3],'full');

				$post_date = $pdate[4]."-".$mm."-".trim($pdate[2])." ".trim($pdate[5]);
			...
			{ 	
				...
				if($i < $parsed_posts_count-1)
				{
					...
					$c_title = $c->find('td[valign=middle] a',1);
					...
					$mm = enMonth_decoder($cdate[5],'full');
