<?PHP
	...
	function parse_hometophit($fetch,$debug = false){
		...
		if($parsed_posts_count == 0){			
			...
			$board_msg = $html->find('table[width=100%] div[align=justify]',0);
			$post_body = trim(iconv("tis-620","utf-8//TRANSLIT//IGNORE",$board_msg->plaintext));			
			
			$author = $html->find('table[width=100%] td[width=186]',0);		
			$post_author = trim(iconv("tis-620","utf-8//TRANSLIT//IGNORE",$author->plaintext));
						
			$date_time = $author->next_sibling()->next_sibling()->next_sibling()->next_sibling();
		...
		$comments = $html->find('table[width=100%] table[width=600]');
	...
	$url = "http://www.hometophit.com/hometh/view_board.php?q_no=693&key=%A2%CD%E1%BA%BA%E1%BB%C5%B9%CB%E9%CD%A7%E0%AA%E8%D2";
