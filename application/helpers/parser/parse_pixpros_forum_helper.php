<?php
	function parse_pixpros_forum($fetch,$page,$debug=false)
	{
		$log_unit_name = 'parse_pixpros_forum';
		...
		else
		{
			if($parsed_posts_count == 0 && $page->sub_comment == 0) // No early post and not a sub comment page
			{
				...
				$post_date = $pdate[2]."-".$mm."-".trim($pdate[0])." ".trim($pdate[3]);
			...
			{ 	
				...
				if($i < $parsed_posts_count-1)
				{
					...
					$c_body = $c->find('div[id^=post_message]',0);
					...
					$comment_date = $cdate[2]."-".$mm."-".$cdate[0]." ".$cdate[3];
					//}
