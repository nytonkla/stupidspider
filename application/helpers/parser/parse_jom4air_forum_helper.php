<?php
	function parse_jom4air_forum($fetch,$page,$debug=false)
	{
		$log_unit_name = 'parse_jom4air_forum';
		...
		else
		{
			...
			{
				$main_content = $html->find('td[class=content] span[class=h1]',0);
				...
				$author = $html->find('span[class=h3]',0);
...
//				$post_date = $yy."-".$mm."-".trim($pd[1])." ".trim($pd[5]);
				$post_date = $pd[$pd_len-2].' '.$pd[$pd_len-1];
